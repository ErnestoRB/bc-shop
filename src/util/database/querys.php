...
{
    return "SELECT idProducto, p.nombre, c.nombre as categoria, descripcion, existencia, precio, imagen FROM productos as p JOIN categoria as c on c.idCategoria = p.idCategoria WHERE p.idCategoria = ? ORDER BY agregado";
}

function getProductsFromVenta()
{
    return "SELECT v.idVenta, p.imagen, p.nombre, IFNULL(vp.precio_oferta,p.precio) precio, vp.cantidad, (IFNULL(vp.precio_oferta,p.precio) * vp.cantidad) total, c.nombre categoria from venta v JOIN venta_producto vp on v.idVenta = vp.idVenta JOIN productos p on p.idProducto = vp.idProducto JOIN categoria c on c.idCategoria = p.idCategoria WHERE v.idVenta = ?";
}
